...
interface GoalSheet { id: string; status: string; submitted_at: string | null; }
interface Goal {
...
const EmployeeDashboard = () => {
  ...
  const canSubmit = sheet && sheet.status === 'draft' && goals.length > 0 && goals.length <= 8;
    ...
    <div className="space-y-6">
        ...
        <div className="flex justify-between items-start flex-wrap gap-4">
          <div>
            <h2 className="text-2xl font-bold text-slate-800">
              My Goal Sheet — {activeCycle?.name || 'No Active Cycle'}
            </h2>
            <p className="text-slate-500 mt-1">
              Welcome back, <span className="font-medium text-slate-700">{user.full_name || 'User'}</span>
              {user.designation && <span className="text-slate-400"> · {user.designation}</span>}
            </p>
            {sheet && (
              <span className={`mt-2 inline-block px-3 py-1 rounded-full text-xs font-medium ${statusColors[sheet.status] || 'bg-slate-100 text-slate-600'}`}>
      ...
      {sheet?.status === 'rework_requested' && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-amber-800 text-sm">
          <strong>Rework Requested:</strong> Your manager has requested changes to your goal sheet.
        </div>
      )}
